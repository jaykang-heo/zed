...
impl EditAgent {
    ...
    async fn overwrite_with_chunks_internal(
        ...
        if first_chunk {
            anyhow::bail!(
                "The model did not produce any file content. \
...
mod tests {
    ...
    #[gpui::test]
    async fn test_overwrite_no_content_is_error(cx: &mut TestAppContext) {
        let agent = init_test(cx).await;
        let buffer = cx.new(|cx| Buffer::local("abc\ndef\nghi", cx));
        ...
        assert!(
            result.is_err(),
            "expected an error when no content was produced"
        );
        assert_eq!(drain_events(&mut events).as_slice(), []);
        assert_eq!(
            buffer.read_with(cx, |buffer, _| buffer.snapshot().text()),
