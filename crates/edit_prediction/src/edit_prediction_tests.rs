...
impl RequestChannels {
    fn assert_no_predict_request(&mut self) {
        assert!(
            self.predict.try_next().is_err(),
            "expected no prediction request, but one was pending"
        );
    }

    async fn next_predict_request(
        &mut self,
    ) -> (
        PredictEditsV3Request,
        oneshot::Sender<PredictEditsV3Response>,
...
async fn test_diagnostic_prediction_deduplication(cx: &mut TestAppContext) {
    let (ep_store, mut channels) = init_test_with_fake_client(cx);
    let fs = FakeFs::new(cx.executor());
    fs.insert_tree(
        "/root",
        json!({
    ...
    async fn await_prediction(channels: &mut RequestChannels, cx: &mut TestAppContext, diff: &str) {
        let (request, response_sender) = channels.next_predict_request().await;
        response_sender
    ...
    fn push_and_expect_no_request(
        ...
        cx.run_until_parked();
        channels.assert_no_predict_request();
    }
    ...
    await_prediction(&mut channels, cx, diff_change_x).await;
    ...
    await_prediction(&mut channels, cx, diff_fix_z).await;
    reject_current(&ep_store, &project, cx);
    ...
    await_prediction(&mut channels, cx, diff_change_y).await;
    reject_current(&ep_store, &project, cx);
    ...
    await_prediction(&mut channels, cx, diff_rename_main).await;
    reject_current(&ep_store, &project, cx);
