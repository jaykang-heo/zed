...
use agent_settings::{
    AgentSettings, CompiledRegex, ToolPermissionRulesOutcome, ToolPermissions,
    check_tool_permission_rules, check_tool_rules,
};
...
impl From<ToolPermissionRulesOutcome> for ToolPermissionDecision {
    fn from(outcome: ToolPermissionRulesOutcome) -> Self {
        match outcome {
            ToolPermissionRulesOutcome::Allow => ToolPermissionDecision::Allow,
            ToolPermissionRulesOutcome::Deny(msg) => ToolPermissionDecision::Deny(msg),
            ToolPermissionRulesOutcome::Confirm => ToolPermissionDecision::Confirm,
        }
    }
}
...
impl ToolPermissionDecision {
    ...
    pub fn from_input(
        ...
        // For non-terminal tools, delegate entirely to the shared pattern-matching logic.
        if tool_name != TerminalTool::NAME {
            return check_tool_permission_rules(tool_name, inputs, permissions, true).into();
        }

        // Terminal tool: need shell-specific logic before delegating.
        ...
        if !shell_kind.supports_posix_chaining() {
            ...
            return check_tool_rules(tool_name, inputs, rules, false, permissions.default).into();
        }
